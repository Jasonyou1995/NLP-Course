...
class Classifier(object):
  ...
  def __init__(self,num_feats,hidden_size,is_training=True):
    ...
    with graph.as_default():
      ...
      dim = 5     # number of columns for the weight of the first layer
      dim2 = 3    # number of columns for the weight of the second layer
      w1 = tf.get_variable("w1",
                          [num_feats,dim],    # dim can vary, num_feats is vocab_size
                          tf.float32)
      b1 = tf.get_variable("b1",
                          [dim],           # a row vector of the shape: 1 x dim
                          tf.float32)
      ...
      h1 = tf.tanh( tf.nn.xw_plus_b(self._feats, w1, b1) )  # define one more hidden layer
...
ONE_HOT_VOCAB_SIZE = 10000  # 10000
HIDDEN_SIZE = 225           # 200
